<template>
  <v-app-bar color="primary" dark app>
    ...
    <v-spacer />
    <v-toolbar-items>
      <v-btn icon large text @click="$emit('sidebar')">
        <v-avatar size="30px">
...
export default {
  ...
  computed: {
    ...mapGetters(["user", "getAvatar", "getUsername"]),
    toolbarColor() {
    ...
    breadcrumbs() {
      ...
      return matched.filter(route => !route.meta.hiddenInMenu)
        .map((route, index) => {
            ...
            to =
              index === matched.length - 1
                ? this.$route.path.replace(":id", url[1])
                  .replace(":hash", url[4])
                : route.path.replace(":id", url[1])
                .replace(":hash", url[4]) || route.redirect;
          } else {
            to =
              index === matched.length - 1
                ? this.$route.path
