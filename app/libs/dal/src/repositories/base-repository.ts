...
export abstract class BaseRepository<T extends { id?: unknown }> {
  ...
  constructor(private target: EntityTarget<T>, private manager: EntityManager) {
  ...
  async getOneAndCheckIfExistsBy(filter: FindOptionsWhere<T>, options?: FindOneOptions<T>) {
