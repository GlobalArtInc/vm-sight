...
export class DockerDataSharingService {
  ...
  private endpointContainersSource = new BehaviorSubject<Container[] | null>(
    null
  );
  endpointInfo$ = this.endpointInfoSource
    .asObservable()
    .pipe(filter(containers => containers !== null));
  endpointContainers$ = this.endpointContainersSource
    .asObservable()
    .pipe(filter(containers => containers !== null));
